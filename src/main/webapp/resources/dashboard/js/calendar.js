...
{
	return (dateObj.getYear() % 100) + (((dateObj.getYear() % 100) < 39) ? 2000 : 2010);
}
...
function showCalender(elPos, tgtEl)
{
	...
	{
		if (document.tCTrHistoryActionForm.elements[tgtEl])
		{
			targetEl = document.tCTrHistoryActionForm.elements[tgtEl];
		}
	}
	...
	calTable.style.left = 46+'px';		
	calTable.style.top = 220+'px';			
...
{
	positions[0] += el.offsetLeft-5.2;
	positions[1] += el.offsetTop+1.7;
	if (el.offsetParent)
